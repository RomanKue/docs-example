  ...
  if (isMagicComment(comment, magicComments.review)) {
    if (getIssueState(issue) === issueState.waitingForReview) {
      await addSimpleComment(issue, user =>
        `@${user} I understood that you want me to review your issue again, I will start with that right away... `
      );
      await reviewIssue(issue);
    } else {
      await addSimpleComment(issue, user =>
        `@${user} I understood that you want me to review your issue again.
    ...
    if (getIssueState(issue) === issueState.waitingForApproval) {
      await addSimpleComment(issue, user =>
        `@${user} I understood that you want me to approve the issue.`
      );
      await approveIssue(issue, comment);
    } else {
      await addSimpleComment(issue, user =>
        `@${user} I understood that you want to approve the issue.
    ...
    const possibleComments =
      ...
      ``;
    await addSimpleComment(issue, user =>
      `@${user} I am not sure I can help you with your request. Try one of the following comments:\n\n${possibleComments}`
    );
  }
};
