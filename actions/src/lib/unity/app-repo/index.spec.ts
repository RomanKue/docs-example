...
import {AppMember, AppSpecV1Beta1} from '../app-spec.js';
...
import {SimpleUser} from '../../github/api/issues/response/issue.js';
...
describe('index', () => {
  ...
  describe('createRepository', () => {
    it('should create repo when called with v1beta1 app', async () => {
      ...
      const repository = await createRepository(newAppIssue, v1beta1);
  ...
  describe('removeOrgMembers', () => {
    let orgMember: AppMember;
    let nonOrgMember: AppMember;
    beforeEach(() => {
      orgMember = partialMock({qNumber: 'orgMember'});
      nonOrgMember = partialMock({qNumber: 'nonOrgMember'});
    ...
    it('should not remove member when user is not org member', async () => {
      ...
      expect(appMembers[0]).toEqual(expect.objectContaining({qNumber: 'nonOrgMember'}));
    });
